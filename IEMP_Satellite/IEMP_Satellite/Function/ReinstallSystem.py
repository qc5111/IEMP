...
class ReinstallSystem:
    ...
    def CopyPEFile(self, PE7zFilePath, VolumeLetter):
        ...
        p_CMD = pipe.RemotePipe(self.IP, self.Port,
                                DynamicPassword.DynamicPassword(self.Password, 8) + "cmd.exe".encode("GB2312"))
    ...
    def BootFromPE(self, VolumeLetter):
        ...
        p_CMD.AutoExec(
            'bcdedit /set {ffffffff-8d96-11de-8e71-ffffffffffff} ramdisksdidevice partition=%s:' % VolumeLetter)
        p_CMD.AutoExec('bcdedit /set {ffffffff-8d96-11de-8e71-ffffffffffff} ramdisksdipath \\Boot\\boot.sdi')
        p_CMD.AutoExec(
            'bcdedit /set {ffffffff-8d96-11de-8e71-fffffffffffe} device ramdisk=[%s:]\\Sources\\boot.wim,{ffffffff-8d96-11de-8e71-ffffffffffff}' % VolumeLetter)
        p_CMD.AutoExec(
        ...
        p_CMD.AutoExec(
            'bcdedit /set {ffffffff-8d96-11de-8e71-fffffffffffe} osdevice ramdisk=[%s:]\\Sources\\boot.wim,{ffffffff-8d96-11de-8e71-ffffffffffff}' % VolumeLetter)
