    ...
    def GetFileList(self,Path):
        ...
        while True:
            ...
            if FileList[Pos] == 4:
                ...
                Pos+=4
                FileDictArr.append({"Type":4,"Name":FileList[Pos+2:Pos+2+FileList[Pos+1]].decode(self.Encoding),"Mtime":Mtime})
                Pos+=FileList[Pos+1]+2
            elif FileList[Pos] == 8:
                #print(Pos,FileList[Pos+2:Pos+1+FileList[Pos+1]],FileList[Pos+1])
                Mtime = struct.unpack("<I",FileList[Pos+1:Pos+5])[0]
                Pos+=4
                Size = struct.unpack("<Q",FileList[Pos+2:Pos+1+FileList[Pos+1]]+b"\x00"*(9-FileList[Pos+1]))[0]
                Pos+=FileList[Pos+1]
                FileDictArr.append({"Type":8,"Name":FileList[Pos+2:Pos+2+FileList[Pos+1]].decode(self.Encoding),"Mtime":Mtime,"Size":Size})
                Pos+=FileList[Pos+1]+2
                #print(Size)
            #print(Pos,MaxPos)
