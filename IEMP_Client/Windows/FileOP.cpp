...
char * GetFileList(char * Path){//记得free
    ...
    while ((entry = readdir(pDir)) != NULL){
        ...
        _stati64(NowFilePath, &File);
        //printf("%s,",NowFilePath);//entry->d_name
        //printf("%lo,",File.st_mode);
...
char * GetDrivers(){
    ...
    for(char letter = 'A'; letter <= 'Z'; letter++){
        ...
        DiskType = GetDriveType(rootPath);
        //printf("rootPath:%s\n",rootPath);
        //printf("DiskType:%d\n",DiskType);
        if(DiskType>1){
            Flist[Nowlen] = DiskType+128;
            Nowlen++;
            Flist[Nowlen] = 3;
            Flist[Nowlen+1] = 0;
            Nowlen++;
            strcat(Flist+Nowlen,rootPath);
            Nowlen += 3;
            //printf("rootPath:%s\n",rootPath);
            //printf("DiskType:%d\n",DiskType);
        }
